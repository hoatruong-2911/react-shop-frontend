...
const ProductListPage = () => {
  ...
  useEffect(() => {
    ...
    if (selectedBrand) {
      result = result.filter((p) => p.brandId === parseInt(selectedBrand));
    }
        ...
        <table className="min-w-full bg-white border">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="py-3 px-6 text-left text-xs font-medium text-gray-500 uppercase">Thương hiệu</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {currentProducts.map((product) => (
              <tr key={product.id} className="hover:bg-gray-50">
                ...
                <td className="py-4 px-6 text-sm text-gray-500">{product.brandName}</td>
